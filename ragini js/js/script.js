...
let displayData = () => {
    ...
    storage.forEach((rec) => {
        ...
        <td>${rec.age}</td>
        <td>${rec.state}</td>
        <td><button class="btn btn-primary" onClick="return singleRecord(${rec.id})" ><i class="bi bi-pencil-fill"></i></button> || <button class="btn btn-danger" onClick="return deletRec(${rec.id})"><i class="bi bi-trash3-fill"></i></button></td>
        </tr>`
    })
}
displayData();